...
//! - `EventuallyConsistentRleBits` - Thread-local with eventual consistency
//!
...
#[derive(Debug, Clone)]
pub struct RleBits {
    runs: [usize; 200], // Alternating run lengths, starting with 0s at index 0
    length: usize, // Upper limit of values, exclusive (ie, 0-9 is represented by 10)
}

impl RleBits {
    pub const fn new(limit: usize) -> Self {
        let mut runs = [0usize; 200];
        runs[0] = limit;
        RleBits { runs, length: limit }
    }
    ...
    pub fn set_range(&mut self, mut index: usize, mut len: usize, value: u8) -> Result<(), RleBitsError> {
        if (index + len < index) || (index + len > self.length) {
        ...
        while len > 0 && i < self.runs.len() {
            ...
            if index < next { // Overlap between the range and the current run
                if (i&0x1) == value.into() {
                    // The current run has the same value as the desired range
                    if index + len <= next { // Entire range is within this run
                        return Ok(());
                    }
                ...
                if index == start {
                    ...
                    if i != 0 {
                        ...
                        i = i-1; // repeat analysis of newly expanded prior run
                        continue;
                    }
