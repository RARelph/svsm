...
static GLOBAL_BITS: ThreadSafeRleBits = ThreadSafeRleBits::new(1_usize << 52);

#[cfg(feature = "thread-safe")]
fn main() {
    println!("Global ThreadSafeRleBits Example (const fn initialization)");
    println!("Bit array limit: 2^52 = {}", 1_usize << 52);

    // Access the global bit array directly - no initialization needed!
    println!("Initial sanity check: {}", GLOBAL_BITS.sanity_check());

    // Set some bits across a huge range
    GLOBAL_BITS.set(0, 1).unwrap();
    GLOBAL_BITS.set(1000, 1).unwrap();
    GLOBAL_BITS.set(1_000_000, 1).unwrap();
    GLOBAL_BITS.set(1_000_000_000, 1).unwrap();
    GLOBAL_BITS.set((1_usize << 52) - 1, 1).unwrap(); // Last possible bit

    println!("After setting sparse bits:");
    println!("Bit 0: {:?}", GLOBAL_BITS.get(0));
    println!("Bit 1000: {:?}", GLOBAL_BITS.get(1000));
    println!("Bit 1,000,000: {:?}", GLOBAL_BITS.get(1_000_000));
    println!("Bit 1,000,000,000: {:?}", GLOBAL_BITS.get(1_000_000_000));
    println!("Last bit (2^52-1): {:?}", GLOBAL_BITS.get((1_usize << 52) - 1));
    println!("Sanity check: {}", GLOBAL_BITS.sanity_check());

    // Set a large range efficiently (this creates fewer run segments)
    println!("\nSetting range 10,000,000 to 10,100,000 (100,000 bits)...");
    GLOBAL_BITS.set_range(10_000_000, 100_000, 1).unwrap();
    println!("Sanity check after range set: {}", GLOBAL_BITS.sanity_check());
    ...
    for thread_id in 0..4 {
        let handle = thread::spawn(move || {
            ...
            GLOBAL_BITS.set_range(base, 500_000, 1).unwrap();

            println!("Thread {} set range {}-{}", 
                    thread_id, base, base + 500_000);
        });
        handles.push(handle);
    }
    ...
    println!("\nFinal sanity check: {}", GLOBAL_BITS.sanity_check());

    // Demonstrate atomic operations on global
    GLOBAL_BITS.with_lock(|bits| {
        // Perform multiple operations atomically
        bits.set_range(50_000_000, 1_000_000, 1).unwrap();
        println!("Atomic operation: set 1M bits starting at 50M");
    });

    println!("Final sanity check after atomic ops: {}", GLOBAL_BITS.sanity_check());
    ...
    println!("Current runs used: {}", GLOBAL_BITS.sanity_check());

    println!("\nNote: This uses const fn for true static initialization,");
    println!("with zero runtime initialization cost - perfect for kernel environments like SVSM!");
}
