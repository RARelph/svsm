...
fn main() {
    // Basic no_std usage
    let mut basic_bits = RleBits::new(100);
    basic_bits.set(10, 1).unwrap();
    basic_bits.set_range(20, 30, 1).unwrap();
    ...
    #[cfg(feature = "thread-safe")]
    {
        let safe_bits = ThreadSafeRleBits::new(100);
        safe_bits.set(15, 1).unwrap();
        safe_bits.set_range(40, 20, 1).unwrap();
        ...
        safe_bits.with_lock(|bits| {
            bits.set_range(70, 10, 1).unwrap();
            bits.set_range(85, 10, 1).unwrap();
        });
