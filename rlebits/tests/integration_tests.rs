...
fn test_set_range_leading_zeroes_less_than_len() {
    let mut rle = RleBits::new(10);
    rle.set_range(0, 5, 0).unwrap(); // No-op, but explicit
    rle.set_range(0, 7, 1).unwrap();
    for i in 0..7 {
        assert_eq!(rle.get(i), Some(1));
    }
    for i in 7..10 {
        assert_eq!(rle.get(i), Some(0));
    }
    assert_eq!(rle.get_run(0), 0);
    assert_eq!(rle.get_run(1), 7);
    assert_eq!(rle.get_run(2), 3);
...
fn test_set_bit1_then_bit0() {
    ...
    assert_eq!(rle.get_run(0), 0);
    assert_eq!(rle.get_run(1), 2);
    assert_eq!(rle.get_run(2), 8);
    assert_eq!(rle.sanity_check(), 3);
}
...
fn test_set_range_leading_zeroes_greater_len() {
    let mut rle = RleBits::new(10);
    rle.set_range(0, 11, 1).unwrap_err();
}
...
fn test_randomized_set_range() {
    ...
    for _ in 0..200 {
        ...
        let range = if max_len == 0 { 1 } else { rng.random_range(1..=max_len as u64) as usize };
        let value = rng.random_range(0..=1);
...
fn test_zero_run_followed_by_nonzero_failure() {
    let mut rle = RleBits::new(36);
    rle.set(35, 1).unwrap();
    rle.set(34, 1).unwrap();
    assert!(rle.sanity_check() == 2);
    
    let mut seen_zero = false;
    for i in 1..200 { // Check reasonable number of runs
        let run = rle.get_run(i);
        if run == 0 {
            seen_zero = true;
        } else if seen_zero {
        ...
        if seen_zero && run == 0 {
            ...
            for j in (i+1)..std::cmp::min(i+5, 200) {
                if rle.get_run(j) != 0 {
                    all_zero = false;
                    break;
                }
...
proptest! {
    #[test]
    fn prop_set_get_matches(length in 1usize..100, idx in 0usize..100, value in 0u8..=1) {
        let mut rle = RleBits::new(length);
        if idx < length {
            rle.set(idx, value).unwrap();
    ...
    #[test]
    fn prop_set_range_get_matches(length in 1usize..100, start in 0usize..100, range in 1usize..20, value in 0u8..=1) {
        let mut rle = RleBits::new(length);
    ...
    #[test]
    fn prop_no_zero_run_followed_by_nonzero(length in 1usize..100, ops in proptest::collection::vec((0usize..100, 0u8..=1), 1..50)) {
        let mut rle = RleBits::new(length);
...
fn test_partial_leading_ones_triggers_move_runs_up() {
    let mut rle_bits = RleBits::new(10);
    rle_bits.set_range(0, 3, 1).unwrap();
    for i in 0..3 {
        assert_eq!(rle_bits.get(i), Some(1));
    }
    for i in 3..10 {
        assert_eq!(rle_bits.get(i), Some(0));
    }
...
fn test_runs_array_overflow() {
    let n = 250;
    let mut rle_bits = RleBits::new(n);
    let mut got_error = false;
    for i in 0..n {
        let res = rle_bits.set(i, (i % 2) as u8);
        if res.is_err() {
            got_error = true;
            break;
        }
    }
    assert!(got_error, "Should have returned an error due to runs array overflow");
}

#[test]
fn test_sequential_set_all_ones() {
    let mut rle_bits = RleBits::new(32);
    for i in 0..32 {
        rle_bits.set(i, 1).unwrap();
        assert_eq!(rle_bits.get(i), Some(1));
        for j in 0..=i {
            assert_eq!(rle_bits.get(j), Some(1));
        }
        for j in (i+1)..32 {
            assert_eq!(rle_bits.get(j), Some(0));
        }
...
fn test_sequential_set_alternate() {
    let mut rle_bits = RleBits::new(16);
    for i in 0..16 {
        rle_bits.set(i, (i % 2) as u8).unwrap();
        assert_eq!(rle_bits.get(i), Some((i % 2) as u8));
        rle_bits.sanity_check();
    }
    for i in 0..16 {
        assert_eq!(rle_bits.get(i), Some((i % 2) as u8));
...
fn test_sequential_set_flip_flop() {
    let mut rle_bits = RleBits::new(8);
    for i in 0..8 {
        rle_bits.set(i, 1).unwrap();
        assert_eq!(rle_bits.get(i), Some(1));
        rle_bits.sanity_check();
...
#[test]
fn test_sanity() {
    let rle_bits = RleBits::new(100);
    assert_eq!(rle_bits.sanity_check(), 1);
    assert_eq!(rle_bits.get(99), Some(0));
    assert_eq!(rle_bits.get(100), None);
}

#[test]
fn test_all_ones() {
    let mut rle_bits = RleBits::new(50);
    rle_bits.set_range(0, 50, 1).unwrap();
    for i in 0..50 {
        assert_eq!(rle_bits.get(i), Some(1));
    }
    assert_eq!(rle_bits.sanity_check(), 2);
...
fn test_set_range_edges() {
    let mut rle_bits = RleBits::new(20);
    rle_bits.set_range(0, 5, 1).unwrap();
    rle_bits.set_range(15, 5, 1).unwrap();
    for i in 0..5 {
        assert_eq!(rle_bits.get(i), Some(1));
    }
    for i in 5..15 {
...
fn test_set_range_middle() {
    let mut rle_bits = RleBits::new(20);
    rle_bits.set_range(7, 6, 1).unwrap();
    for i in 0..7 {
        assert_eq!(rle_bits.get(i), Some(0));
    }
    for i in 7..13 {
...
fn test_set_range_overlapping() {
    let mut rle_bits = RleBits::new(30);
    rle_bits.set_range(5, 10, 1).unwrap();
    rle_bits.set_range(10, 10, 0).unwrap();
    for i in 0..5 {
        assert_eq!(rle_bits.get(i), Some(0));
    }
    for i in 5..10 {
...
#[test]
fn test_large_range() {
    let mut rle_bits = RleBits::new(150);
    rle_bits.set_range(50, 50, 1).unwrap();
    for i in 0..50 {
        assert_eq!(rle_bits.get(i), Some(0));
    }
    for i in 50..100 {
...
fn test_set_range_out_of_bounds() {
    let mut rle_bits = RleBits::new(10);
    rle_bits.set_range(5, 10, 1).unwrap_err();
}

#[test]
fn test_set_invalid_value() {
    let mut rle_bits = RleBits::new(10);
    rle_bits.set(0, 2).unwrap_err();
}

#[test]
fn test_leading_one() {
    let mut rle_bits = RleBits::new(100);
    rle_bits.set(0, 1).unwrap();
    assert!(rle_bits.get(2) == Some(0));
    assert!(rle_bits.get(0) == Some(1));
    assert!(rle_bits.get(4) == Some(0));
    assert!(rle_bits.sanity_check() == 3);
    rle_bits.set(0,0).unwrap();
...
fn test_set_bit3() {
    let mut rle_bits = RleBits::new(100);
    rle_bits.set(3, 1).unwrap();
    assert!(rle_bits.get(2) == Some(0));
    assert!(rle_bits.get(3) == Some(1));
    assert!(rle_bits.get(4) == Some(0));
    assert!(rle_bits.sanity_check() == 3);
}
...
fn test_set_bit99() {
    let mut rle_bits = RleBits::new(100);
    rle_bits.set(99, 1).unwrap();
    assert!(rle_bits.get(98) == Some(0));
    assert!(rle_bits.get(99) == Some(1));
    assert!(rle_bits.get(100) == None);
...
fn test_set_bits3and4() {
    let mut rle_bits = RleBits::new(100);
    rle_bits.set(3, 1).unwrap();
    rle_bits.set(4,1).unwrap();
    assert!(rle_bits.get(2) == Some(0));
    assert!(rle_bits.get(3) == Some(1));
    assert!(rle_bits.get(4) == Some(1));
    assert!(rle_bits.get(5) == Some(0));
    assert!(rle_bits.sanity_check() == 3);
}
...
fn test_set_bit3for2() {
    let mut rle_bits = RleBits::new(100);
    rle_bits.set_range(3, 2, 1).unwrap();
    assert!(rle_bits.get(2) == Some(0));
    assert!(rle_bits.get(3) == Some(1));
    assert!(rle_bits.get(4) == Some(1));
    assert!(rle_bits.get(5) == Some(0));
    assert!(rle_bits.sanity_check() == 3);
    rle_bits.set(3, 0).unwrap();
    assert!(rle_bits.sanity_check() == 3);
    rle_bits.set(4,0).unwrap();
    assert!(rle_bits.sanity_check() == 1);
}
...
fn test_set_bit9for2() {
    let mut rle_bits = RleBits::new(100);
    rle_bits.set_range(9, 2, 1).unwrap();
    rle_bits.set_range(13, 2, 1).unwrap();
    assert!(rle_bits.get(8) == Some(0));
    assert!(rle_bits.get(9) == Some(1));
    assert!(rle_bits.get(10) == Some(1));
    assert!(rle_bits.get(11) == Some(0));
    assert!(rle_bits.get(12) == Some(0));
    assert!(rle_bits.get(13) == Some(1));
    assert!(rle_bits.get(14) == Some(1));
    assert!(rle_bits.get(15) == Some(0));
    assert!(rle_bits.sanity_check() == 5);
    rle_bits.set_range(7, 25, 0).unwrap();
    assert!(rle_bits.sanity_check() == 1);
}
...
mod thread_safe_tests {
    ...
    fn test_thread_safe_basic_operations() {
        let rle = ThreadSafeRleBits::new(100);
    ...
    fn test_multi_threaded_access() {
        let rle = Arc::new(ThreadSafeRleBits::new(1000));
        let mut handles = vec![];
        ...
        for thread_id in 0..4 {
            ...
            let handle = thread::spawn(move || {
                ...
                rle_clone.set_range(start, 250, 1).unwrap();
                
                // Verify the range was set correctly
                for i in start..end {
                    assert_eq!(rle_clone.get(i), Some(1));
                }
            });
            handles.push(handle);
        }
    ...
    fn test_concurrent_read_write() {
        let rle = Arc::new(ThreadSafeRleBits::new(100));
        let mut handles = vec![];
    ...
    fn test_with_lock_atomicity() {
        ...
        let result = rle.with_lock(|bits| {
            bits.set_range(10, 20, 1).unwrap();
            bits.set_range(30, 20, 1).unwrap();
            bits.sanity_check()
        });
        ...
        for thread_id in 0..8 {
            ...
            let handle = thread::spawn(move || {
                ...
                for _ in 0..100 {
                    ...
                    let value = rng.random_range(0..2) as u8;
